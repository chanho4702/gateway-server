...
의존성: `spring-cloud-starter-gateway-server-webflux` + `spring-cloud-starter-netflix-eureka-client`(서비스 디스커버리, LoadBalancer 전이 포함) + `spring-boot-starter-data-redis-reactive`(rate limit) + `spring-cloud-starter-circuitbreaker-reactor-resilience4j`(CB) + `spring-boot-starter-security`/`oauth2-resource-server`(JWT 조기차단). 테스트에만 `spring-boot-starter-webflux`(WebTestClient) 추가.
...
> 포트 맵: gateway 8000 / eureka 8761 / auth 9000 / board 9100 / Keycloak 8080 / Postgres 5433 / myFront 5173.
...
| 라우트 id | 경로 패턴 | → 대상 | 비고 |
|---|---|---|---|
| `board` | `/api/board/**` | `lb://board-service` | 게시판·댓글 |
| `auth-oauth2` | `/oauth2/**` | `lb://auth-server` | OIDC 로그인 시작·콜백 |
| `auth-login` | `/login/**` | `lb://auth-server` | OIDC 리다이렉트 흐름 |
| `auth-api` | `/api/auth/**` | `lb://auth-server` | refresh · logout |
| `auth-jwks` | `/.well-known/**` | `lb://auth-server` | JWKS 공개키 |
| `auth-me` | `/api/me` | `lb://auth-server` | 자체 JWT 기반 사용자 정보 |

라우트 id·lb:// 기본값은 `RouteConfigTest`가 검증하므로, 바꾸면 테스트도 함께 갱신한다.
`lb://` 뒤의 이름은 대상 서비스의 `spring.application.name`과 일치해야 한다(유레카 등록 ID).

> **새 서비스 추가**: 대상 서비스에 eureka-client 의존성 + `spring.application.name` 확인, 게이트웨이 `application.yml`에 라우트 한 줄(`lb://서비스명`) 추가로 끝. 게이트웨이 재기동 외 설정 변경 불필요.

 ...
 * </table>
 *
 * <p><b>모드를 하나 더 늘릴 때는 프론트도 함께 고친다.</b> wiki-front의 {@code parseMode}는 아는 값만
 * 통과시키는 화이트리스트라, 게이트웨이만 새 모드를 알면 그 설치는 화면에서 {@code lite}로 좁혀진다
 * (능력이 조용히 꺼진 채로 도는 것보다 안전하도록 일부러 그렇게 돼 있다). 값을 늘리는 커밋은
 * wiki-front의 화이트리스트·안내 문구와 같은 시점에 나가야 한다.
 *
